...
const MemberHOD = () => {
  ...
  useEffect(() => {
    const fetchHODs = async () => {
      setLoading(true);
      try {
        const hodData = await ApiFunction.getUsersByRole("hod");
        setHods(hodData);
      } catch (error) {
  ...
  const filteredHOD = hods.filter((hod) =>
  ...
  if (loading) {
    return (
      <div className="w-full h-fit mr-2 border border-gray-300 rounded-lg p-4 text-center">
        ...
        <p className="mt-2 text-gray-600">Loading HODs...</p>
      </div>
    );
  }
            ...
            {filteredHOD.length > 0 ? (
              filteredHOD.map((hod, index) => (
                <tr key={index} className="hover:bg-gray-50">
                  <td className="border border-gray-300 px-4 py-2 text-center">{hod.department}</td>
                  <td className="border border-gray-300 px-4 py-2 text-center">{hod.name}</td>
                  <td className="border border-gray-300 px-4 py-2 text-center">{hod.phone}</td>
                  <td className="border border-gray-300 px-4 py-2 text-center">{hod.email}</td>
                </tr>
              ))
            ) : (
              <tr>
